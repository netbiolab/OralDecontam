...
[zunuan@nodea2 3.recoveredTP_script]$ cat 2-2.polyphen_partially_masked.sh 
#!/bin/bash

REFERENCE="2.no_alt_GRCh38"
CALLER="5.RecoveredTP"
PREV="1-2.isec_partially_masked"
NEXT="2-2.polyphen_partially_masked"
...
for ORAL_SOURCE in "${ORAL_SOURCE_LIST[@]}"; do
  for PGPC in "${PGPC_LIST[@]}"; do
    ...
    COMPLETELY_MASKED_DIR="${BASE_DIR}/${PREV}/HROM_yesambi_${PGPC}/${ORAL_SOURCE}"
    COMPLETELY_MASKED=$(find "${COMPLETELY_MASKED_DIR}" -maxdepth 1 -type f -name "partially_masked.vcf.gz" | head -n 1)
    ...
    vep --cache -i ${COMPLETELY_MASKED} --plugin PolyPhen_SIFT -o ${OUT_DIR}/partially_maked.txt
  done
done
